...
function LoadWeapons()
	...
	for k, v in pairs( Load ) do
		...
		ent:SetPos( v.pos + Vector(0,0,20) )
		ent:SetAngles( v.ang )
		ent:Spawn()
...
function ENT:PhysicsCollide(data,phys) 
	local theEntity = phys:GetEntity()
	theEntity:SetVelocity(Vector(0,0,0))
end
...
hook.Add( "OnRoundSet", "Load Weapons", function( round )
	...
	LoadWeapons()
	timer.Create("weapontimer", 35, 0, function() LoadWeapons() end)

end )

concommand.Add( "add_weapon", function( ply, cmd, args )
...
concommand.Add( "save_weapon", function( ply, cmd, args )
